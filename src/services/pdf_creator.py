...
class PDF(FPDF):
    """Custom FPDF class for generating invoices."""
    ...
    def address(self, text1: str, data: List[str], x: float, y: float) -> None:
        """
        Prints the address block (Seller or Buyer).

        :param text1: The title (e.g., "Seller: ")
        :param data: List of address lines (e.g., name, street, city)
...
def create_pdf(data: Dict[str, Any], file_name: str):
    ...
    # seller
    pdf.address("Seller: ", data["seller_addr"], 10, 60)

    # Buyer
    pdf.address("Buyer: ", data["customer_addr"], 120, 60)
